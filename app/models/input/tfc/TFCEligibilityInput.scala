...
case class TFCClaimant(
  ...
  def totalIncome: BigDecimal = {
    ...
    getTotalTFCIncome(
      currentEmployment.getOrElse(ConfigConstants.defaultAmount),
  ...
  private def getTotalTFCIncome(employmentIncome: BigDecimal, otherIncome: BigDecimal, pension: BigDecimal) =
...
object TFCClaimant {
  ...
  implicit val claimantReads: Reads[TFCClaimant] =
    (JsPath \ "currentIncome")
      ...
      .and((JsPath \ "disability").read[TFCDisability])
      .and((JsPath \ "carersAllowance").read[Boolean].orElse(Reads.pure(false)))
      .and((JsPath \ "minimumEarnings").read[TFCMinimumEarnings])
