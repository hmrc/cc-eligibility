...
class SchemeResultsBuilderSpec extends CCConfigSpec with MockitoSugar {
  ...
  "SchemeResultsBuilder" must {
    ...
    " throw an exception" when {
      "trying to build a scheme object and ESC claimant is missing" in
        intercept[Exception] {
          Scheme(name = SchemeEnum.ESCELIGIBILITY, amount = BigDecimal(0.00), escClaimantEligibility = None)
        }
    }
  ...
  val calcOutputValueAll: Option[CalculatorOutput] = Some(
    CalculatorOutput(Some(BigDecimal(1000)), Some(BigDecimal(1000)))
  )
...
//Values from eligibility
  ...
  val tfcOutputCChild = TFCOutputChild(
    id = 0,
    qualifying = true,
    from = None,
    until = None,
  ...
  val tfcEligibilityOutputRolloutTrue = TFCEligibilityOutput(
    from = LocalDate.now(),
    until = LocalDate.now(),
    householdEligibility = true,
    periods = List(
...
//Values for schemeResults that have already been calculated
  val escSchemeInput = Scheme(
    name = SchemeEnum.ESCELIGIBILITY,
    amount = BigDecimal(10),
    escClaimantEligibility = Some(EscClaimantEligibility(true, true))
  )
...
//values for expected outputs
  val escSchemeOutputZero = Scheme(
    name = SchemeEnum.ESCELIGIBILITY,
    amount = BigDecimal(0.0),
    escClaimantEligibility = Some(EscClaimantEligibility(true, true))
  )

  val escSchemeOutput = Scheme(
    name = SchemeEnum.ESCELIGIBILITY,
