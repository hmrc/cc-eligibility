  ...
  "HHToESCEligibilityInput" must {
    ...
    "convert Household into ESCEligibilityInput" when {
        ...
        val res = ESCEligibilityInput(
          List(
            ESCTaxYear(
              ...
              List(ESCClaimant(false, true, Some(ESCIncome(Some(30000.0), Some(200.0), Some("1100L"))))),
              List()
            ),
            ESCTaxYear(
              HelperManager.determineApril6DateFromNow(LocalDate.now()),
      ...
      "given a household with parent not working, partner working and single child" in {
        ...
        val household = Household(
          children = children,
          parent = Claimant(currentYearlyIncome = None, escVouchers = Some(YesNoUnsureEnum.NO)),
          partner = Some(
            Claimant(
              currentYearlyIncome = Some(Income(Some(30000.00), Some(200.00), Some(1500.00), None, Some("1100L"))),
              escVouchers = Some(YesNoUnsureEnum.YES)
          ...
          List(
            ESCTaxYear(
              ...
              List(
                ...
                ESCClaimant(true, true, Some(ESCIncome(Some(30000.0), Some(200.0), Some("1100L"))))
              ),
              List(
                ESCChild(
                  1,
        ...
        val res = ESCEligibilityInput(
          List(
            ESCTaxYear(
              ...
              HelperManager.determineApril6DateFromNow(LocalDate.now()),
              List(
                ESCClaimant(false, false, Some(ESCIncome(Some(30000.0), Some(200.0)))),
                ESCClaimant(true, false, Some(ESCIncome(Some(30000.0), Some(200.0))))
              ),
              List()
            ),
            ESCTaxYear(
        ...
        val res = ESCEligibilityInput(
          List(
            ESCTaxYear(
              LocalDate.now(),
              HelperManager.determineApril6DateFromNow(LocalDate.now()),
              List(ESCClaimant(false, false, Some(ESCIncome(Some(30000.0), Some(200.0), Some("1200L"))))),
              List()
            ),
            ESCTaxYear(
              HelperManager.determineApril6DateFromNow(LocalDate.now()),
